...
//////////////////////////////////////////////////////////
static void DirectoryList(const string&path, vector<string> &doc, vector<string>& dir){
	_finddata_t file;
	string strfind = path + "//*.*";
	long Handle = _findfirst(strfind.c_str(), &file);
	if (Handle == -1L){
		ERROE_LOG("find erro");
		return;
	}
...
static void Highlight(std::pair<std::string, std::string>as, string key)
{

	{

		string str = as.first;
		string prefix, suffix; int pre = 0;
			int suff=0;
		vector<string>ziduiyin;
		string bijiao = ChineseConvertPinYinAllSpell(str);
		int i = 0;
		while (i < str.size()){
			string mid;
			if (str[i]>127 || str[i] < 0){
				mid = str.substr(i, 2);
				ziduiyin.push_back(ChineseConvertPinYinAllSpell(mid));
				i += 2;
			}
			else{
				mid = str.substr(i, 1);
				ziduiyin.push_back(mid);
				i += 1;
			}
		}
		int loopNUM = bijiao.find(ChineseConvertPinYinAllSpell(key)); int loopNUM2 = loopNUM + key.size();
		int count = 0;
		int sum = 0;
		for (int i = 0; i < ziduiyin.size(); i++){
			if (loopNUM <= 0){
				loopNUM = INT_MAX;
				pre = count * 2 + sum;
			}
			...
			for (int j = 0; j < ziduiyin[i].size(); j++){

				loopNUM--;
				loopNUM2--;

			}
			if (ziduiyin[i].size()>1){
				count++;
			}
			else{
				sum++;
		...
		if (pre > str.size() || pre < 0 || suff<0 || suff>str.size()){
			cout << str << "                          " << as.second << endl;
		}
		else{
			prefix = str.substr(0, pre);
			suffix = str.substr(suff, string::npos);
			printf("%s",prefix.c_str());
			key = str.substr(pre, suff - pre);
			ColourPrintf(key.c_str());
			printf("%-50s%-50s\n", suffix.c_str(), as.second.c_str());
		}
	}
}